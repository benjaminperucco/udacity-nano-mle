...
Let $\sigma(f_j, d_i)$ the number of occurances of feature $f_j$ in document $d_i$. Then we can build a feature matrix
...
\noindent For a specific element in row $i$ and column $j$ (representing the document $d_i$ and feature $f_j$) in the matrix $X_{\sigma}$, we abbreviate by using $\sigma_{ij}$.
